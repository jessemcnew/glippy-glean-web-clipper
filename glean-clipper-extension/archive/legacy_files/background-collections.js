...
// Uses Collections API instead of Indexing API
  ...
  try {
    ...
    if (gleanConfig.enabled && gleanConfig.apiToken && gleanConfig.domain && gleanConfig.collectionId) {
      try {
        ...
        newClip.synced = true;
        console.log('SUCCESS: Clip synced to Glean Collections successfully');
      } catch (error) {
...
async function syncToGleanCollections(clip, config) {
  ...
  // Prepare payload for Collections API
  const payload = {
  ...
  if (!response.ok) {
    if (response.status === 400 && responseText.includes('Error getting request body')) {
      throw new Error('Invalid request format. The Collections API requires a valid collection ID and proper JSON format.');
    }
    throw new Error(`API error (${response.status}): ${responseText}`);
  }
  
  // Collections API returns empty response on success
...
async function testGleanConnection() {
  ...
  // Try a simple Collections API call
