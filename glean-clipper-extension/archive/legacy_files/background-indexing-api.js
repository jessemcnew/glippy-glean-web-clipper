...
async function initializeCollectionsAPI() {
  try {
    ...
    if (config && config.enabled && config.domain && config.apiToken) {
      collectionsAPI = {
        config: config,
        baseUrl: formatDomain(config.domain),
        
        async addClipToCollection(collectionId, clip) {
          const url = `${this.baseUrl}/rest/api/v1/addcollectionitems`;
...
function formatDomain(domain) {
  ...
  // Convert app.glean.com to linkedin-be.glean.com for API access
...
async function attemptCollectionsSync(clip, config, domain) {
  ...
  if (token) {
    ...
    throw new Error('Collections API requires a Client API token');
  }
...
async function testGleanConnection() {
  ...
  console.log('Config check:', {
    ...
    hasDomain: !!config?.domain,
    hasCollectionId: !!config?.collectionId
  });
  
  if (!config) {
    throw new Error('No configuration found. Please save settings first.');
  }
  ...
  if (!config.apiToken) {
    throw new Error('No API token found. Please add your Client API token in settings.');
  }
  
  if (!config.domain) {
    throw new Error('No domain configured. Please add your Glean domain in settings.');
  }
  ...
  // For connection test, just check if we can reach the API
  const testUrl = `https://${domain}/rest/api/v1/collections`;
  
  console.log('🌐 Testing URL:', testUrl);
  console.log('📝 Using token:', config.apiToken.substring(0, 10) + '...');
  ...
  try {
    ...
    if (response.status === 401) {
      throw new Error('Authentication failed. Please check your API token is valid and has COLLECTIONS scope.');
    }
    
    if (response.status === 404) {
      // 404 on collections endpoint might be okay, try a simpler test
      console.log('Collections endpoint not found, but API is reachable');
      return { success: true, message: 'API is reachable (404 on collections list is normal)' };
    }
    
    if (response.ok || response.status === 400) {
      // 400 might mean the API is working but our request format is wrong
